...
function new_task()
{
    ...
    $checkclient = $mysql->query("SELECT * FROM `clients` WHERE `Name_client`='$name_client' AND `Phone_client`='$phone_client'");
    $checkclient = $checkclient->fetch_assoc();

    if (count($checkclient) > 0) 
    	$ID_client = $checkclient['ID_client'];

	 $addclient = $mysql->query("INSERT INTO  `clients` (`ID_client`,`Name_client`,`phone_client`,`referrer`) VALUES('$ID_client','$name_client','$phone_client','$referrer') ");
      $addorder = $mysql->query("INSERT INTO `client_orders` (`ID_order`, `sn_hw_order`, `name_hw_order`, `order_datetime`, `order_end_datetime`, `warranty_order`, `exec_order_start`, `exec_order_end`, `out_order`, `ID_client`, `price_order`, `device_condition` ) VALUES (NULL, '$sn_hw', '$hw_name', NOW(), 'NULL', '$warranty', 'NULL', 'NULL', 'NULL', '$ID_client', '1500', '$device_condition') ");
